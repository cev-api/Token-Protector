...
| Constructor argument capture (`@Inject` on `User.<init>` HEAD) | ❌ Blocked | `MainMixin` poisons arg before any observer sees it |
...
This is blocked. `MainMixin` captures the real JWT at the `Main.main(...) -> new User(...)` call site and passes a fake token into the constructor instead. The real JWT is stashed into `TokenStash.realAccessToken` before any other mod's `@Inject(method="<init>", at=@At("HEAD"))` on `User` can observe it.

`UserMixin` then treats `TokenStash.realAccessToken` as the source of truth during construction, so `TokenVault` still receives the real JWT even though the constructor argument is poisoned. A malicious mod hooking the `User` constructor only sees the fake argument - never the real JWT.
